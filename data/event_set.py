...
class EventSet:
    ...
    def __init__(
        ...
        *,
        ...
        """
        All domain tensors are moved to `device` if provided.
        If device is None, infer from the first domain.
        """

        if not domains:
            raise ValueError("EventSet requires at least one domain.")

        # Infer canonical device
        if device is None:
            for t in domains.values():
                if t.numel() > 0:
                    device = t.device
                    break
            else:
        ...
        self.device = device
        self.domains = { d: t.to(device) for d, t in domains.items() }
        
        self.meta = meta or {}
        self._update_subject_ids()
        self.domain_to_int, self.int_to_domain = self._get_domain_mappings(domain_to_int)
    ...
    def _get_domain_mappings(self, domain_to_int):
        ...
        if domain_to_int is None:
            ...
            self.domain_to_int = dict(domain_to_int)

        self.int_to_domain = {i: d for d, i in self.domain_to_int.items()}
        return self.domain_to_int, self.int_to_domain
    ...
    def for_token(self, domain: str, token_id: int):
        ...
        if len(arr) == 0:
            ...
            return EventSet({d: a.clone() for d, a in self.domains.items()})
    ...
    def merge_domains(self, as_dataframe=True):
        rows = []
    ...
    def pipe(self, func, batch_size=None, device=None, *args, **kwargs):
        ...
        # TODO: Should func get EventSet object instead of dict?
        ...
        for i in range(0, n, batch_size):
            ...
            sub = EventSet(self._subset_by_subjects(batch_sids))
    ...
    def insert_no_event_tokens(self, rate=5, domain="padding", trim_right_padding=True):
        ...
        if trim_right_padding:
            new = new.trim_padding_to_right()

        return new


    def trim_padding_to_right(
        self,
        *,
        padding_domain: str = "padding",
    ) -> "EventSet":
    
        if padding_domain not in self.domains:
            raise ValueError(f"Padding domain '{padding_domain}' not found.")
    
        # 1. Compute right support per subject (as Python floats!)
        support_age = {}
    
        for dname, arr in self.domains.items():
            if dname == padding_domain or arr.numel() == 0:
                continue
            ...
            for sid in torch.unique(sids):
                m = sids == sid
                max_age = float(ages[m].max().item())
                sid_int = int(sid.item())
    
                if sid_int not in support_age:
                    support_age[sid_int] = max_age
                else:
                    support_age[sid_int] = max(support_age[sid_int], max_age)
    
        # 2. Trim per domain, casting support_age to local device
        new_domains = {}
     
        for dname, arr in self.domains.items():
            if arr.numel() == 0:
                new_domains[dname] = arr.clone()
                continue
            ...
            keep = torch.ones(arr.shape[0], dtype=torch.bool, device=device)
    
            for sid_int, max_age in support_age.items():
                sid_t = torch.tensor(sid_int, device=device, dtype=sids.dtype)
                max_age_t = torch.tensor(max_age, device=device, dtype=ages.dtype)
                keep &= ~((sids == sid_t) & (ages > max_age_t))
    
            new_domains[dname] = arr[keep]
    
        return EventSet(
            new_domains,
            domain_to_int=self.domain_to_int,
            meta={**self.meta, "trimmed_to_right_support": True},
        )
    ...
    #   Slow version (just for reference, will be removed in the future)
    ...
    def to_model_inputs(self, device=None):
        ...
        for dname, arr in self.domains.items():
            ...
            if arr.numel() == 0:
                tokens[dname]      = torch.empty(0)
                ages[dname]        = torch.empty(0)
                subject_ids[dname] = torch.empty(0)
    ...
    #TODO: extend to other domains (apart from diseases)
    def get_case_indices(self, disease_token_id: int, min_age: float, max_age: float, sex_token_id: int = None):

        df = self.merge_domains(as_dataframe=True)
